...
import rospy
from eagerx_core.core import RxBridge, RxNode, RxObject, RxGraph, EAGERxEnv
...
if __name__ == '__main__':
    ...
    # Remove component
    graph.remove_component(obj.name, 'sensors', 'observation', remove=True)
    graph.remove_component(obj.name, 'sensors', 'reward', remove=True)
    ...
    env = EAGERxEnv(name='rx', rate=rate, graph=graph, bridge=bridge, is_done_fn=lambda obs, action, steps: steps >= 200)
